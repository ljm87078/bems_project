...
# 실제 IT 장애 데이터 생성 (CPU, 메모리, 네트워크, 서버, 스토리지)
np.random.seed(42)
n_samples = 1000
dates = pd.date_range(start='2023-01-01', periods=n_samples, freq='H')  # 시간별 데이터 생성
data = {
    '날짜': dates,
    ...
    '장애 발생': np.random.choice([0, 1], n_samples, p=[0.95, 0.05])  # 5% 확률로 장애 발생
}
...
model = LogisticRegression(max_iter=1000)  # max_iter 경고 해결
